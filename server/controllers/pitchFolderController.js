...
async function findSignedUpInfluencerForSource(source = {}) {
  // Fidem has no influencer sign-up/accounts — creators only ever exist as
  // discovered YouTube profiles (models/youtube.js). Kept as a no-op stub so
  // callers that branch on "signed up vs. not" continue to degrade cleanly.
  return null;
}
...
function getShareBaseUrl() {
  return process.env.PITCH_FOLDER_SHARE_BASE_URL || 'https://fidem.com/pitch-folder/shared';
}
  ...
  return {
    ...
    shares: Array.isArray(doc.shares)
      ? doc.shares.map((s) => ({
        ...
        sharedBy: serializeMiniAdmin(s.sharedByAdminId),
      }))
      : [],
    assignedCampaign: serializeAssignedCampaign(doc.assignedCampaign),
    createdAt: doc.createdAt,
...
exports.generateShareLink = async (req, res) => {
  try {
    ...
    const brandName = cleanStr(req.body?.brandName);

    // A brand name means "give me a new, brand-tagged link" — never reuse an
    // existing one, so the admin can tell brands apart later. Without a
    // brand name, fall back to the original single-reused-link behavior.
    if (brandName) {
      ...
      // Keep the legacy singular field pointed at the most recent link so
      // any older code path reading doc.share still works.
      doc.share = {
        token,
        url,
        generatedAt: new Date(),
      ...
      const hydrated = await saveAndHydrateFolder(doc);
      const created = hydrated.shares[hydrated.shares.length - 1];
      ...
      return res.json({
        ...
        data: {
          ...
          sharedBy: serializeMiniAdmin(created.sharedByAdminId),
        },
      });
    }
...
exports.getSharedFolder = async (req, res) => {
  try {
    ...
    const doc = await PitchFolder.findOne({
      archivedAt: null,
      $or: [{ 'share.token': token }, { 'shares.token': token }],
    }).lean();
    ...
    // Resolve which specific brand-tagged share matched (if any) so the
    // public page can show "Prepared for {brandName}".
    const matchedBrandShare = Array.isArray(doc.shares)
      ? doc.shares.find((s) => s.token === token)
      : null;
    const resolvedShare = matchedBrandShare || doc.share || {};
    ...
    return res.json({
      ...
      data: {
        ...
        showFullListToBrand: !!doc.showFullListToBrand,
        share: resolvedShare,
        brandName: matchedBrandShare?.brandName || '',
