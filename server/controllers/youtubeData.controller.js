...
const ModashProfile = tryRequireModel(['../models/modash']) || null;

let modashHelpers = {};
try {
  modashHelpers = require('./modashController');
} catch (_) {
  modashHelpers = {};
}
...
const COUNTRY_MATCH_HIGH_CONFIDENCE_THRESHOLD = 65;

// Verified = YouTube explicitly declares the channel's country.
// High/medium = inferred from language/keyword signals (estimateAudienceCountry),
// scored by confidence so callers can fill an exact-N request with the most
// trustworthy inferred matches first instead of treating every match the same.
function getCountryMatchConfidence(doc, targetCountry) {
  const target = normalizeCountryCode(targetCountry);
  if (!target) return 'none';

  const channelCountry = normalizeCountryCode(doc.country);
  if (channelCountry === target) return 'verified';

  const estimatedCountry = normalizeCountryCode(doc.estimatedAudienceCountry);
  if (estimatedCountry !== target) return 'none';

  const confidence = Number(doc.audienceCountryConfidence ?? doc.scores?.audienceCountryConfidence ?? 0);
  return confidence >= COUNTRY_MATCH_HIGH_CONFIDENCE_THRESHOLD ? 'high' : 'medium';
}

const COUNTRY_MATCH_CONFIDENCE_RANK = { verified: 3, high: 2, medium: 1, none: 0 };
...
function buildInfluencerDiscoveryData(doc, context = {}) {
  ...
  return {
    ...
    // Old Apps Script used last 90 days. Fidem now uses last 2 years for activity filtering.
...
function buildMongoFilter({
  ...
  // Multiple selected subscriber tiers (e.g. Nano + Macro, skipping Micro/Mid)
  // — an explicit admin choice, so this is always a hard filter regardless
  // of strictFilters, via an $or of each tier's range.
  if (Array.isArray(subscriberRanges) && subscriberRanges.length) {
    and.push({
      $or: subscriberRanges.map((r) => {
        const range = {};
        if (r.min != null) range.$gte = r.min;
        if (r.max != null) range.$lte = r.max;
...
function creatorListDTO(doc, context = {}) {
  ...
  data.filterMatch = {
    ...
    countryMatchConfidence: getCountryMatchConfidence(doc, context.country),
...
function sortDiscoveryDataForSelectedFilters(items = [], context = {}) {
  ...
  return [...items].sort((a, b) => {
    ...
    if (hasCountryFilter) {
      const aCountry = COUNTRY_MATCH_CONFIDENCE_RANK[a.filterMatch?.countryMatchConfidence] ?? 0;
      const bCountry = COUNTRY_MATCH_CONFIDENCE_RANK[b.filterMatch?.countryMatchConfidence] ?? 0;
...
function subscriberMatchesFilters(subscribers, { minSubscribers, maxSubscribers, subscriberRanges }) {
  if (Array.isArray(subscriberRanges) && subscriberRanges.length) {
    return subscriberRanges.some((r) => {
      if (r.min != null && subscribers < r.min) return false;
      if (r.max != null && subscribers > r.max) return false;
      return true;
    });
  }
  if (minSubscribers != null && subscribers < minSubscribers) return false;
  if (maxSubscribers != null && subscribers > maxSubscribers) return false;
  return true;
}
...
async function runDirectChannelSearch({
  ...
  // Respect the caller's exact-count target directly (no artificial 50-result floor),
  // while still scanning a wide-enough candidate pool to actually find that many matches.
  const targetLimit = Math.max(1, Number(limit) || 25);
  const maxCandidateRows = targetCountry ? Math.min(500, Math.max(targetLimit * 10, 250)) : Math.max(targetLimit * 2, 50);
  ...
  for (const channel of channels) {
    ...
    if (minSubscribers != null || maxSubscribers != null || (Array.isArray(subscriberRanges) && subscriberRanges.length)) {
      const subscriberCount = Number(channel?.statistics?.subscriberCount || 0);
      if (!subscriberMatchesFilters(subscriberCount, { minSubscribers, maxSubscribers, subscriberRanges })) continue;
    }
  ...
  // Fetch/build/save each candidate channel concurrently (bounded) instead of one
  // at a time — with targetLimit now able to reach 300 (Phase 2), a fully sequential
  // loop of per-channel YouTube API calls was slow enough to hit request timeouts.
  const built = await mapWithConcurrency(
    candidateChannels,
    DIRECT_CHANNEL_SEARCH_CONCURRENCY,
    async (channel) => {
      const discoveryInfo = discoveryMap.get(channel.id) || {};
      const uploadsPlaylistId = channel?.contentDetails?.relatedPlaylists?.uploads || null;
      ...
      if (targetCountry && !isCountryMatchForFilter(doc, targetCountry)) return null;
  ...
  for (const item of built) {
    if (!item) continue;
    docs.push(item.doc);
    if (item.saved) refreshedCount += 1;
  }
...
async function browseCreators(req, res) {
  try {
    ...
    const tierRange = getSubscriberTierRange(q.subscriberTier);
    // Multi-select subscriber tiers, e.g. "nano,macro" — each maps to its own
    // range so non-adjacent tiers (skipping the ones in between) work correctly.
    const subscriberTiersList = cleanStr(q.subscriberTiers)
      .split(',')
      .map((t) => t.trim())
      .filter(Boolean);
    // Alternate format: raw numeric "min:max" pairs (comma-separated), e.g.
    // "1000:10000,500000:1000000" — for callers using custom/finer-grained
    // subscriber ranges instead of the named tiers above.
    const subscriberRangePairs = cleanStr(q.subscriberRangePairs)
      .split(',')
      .map((pair) => {
        const [minRaw, maxRaw] = pair.split(':');
        const min = toIntOrNull(minRaw);
        const max = toIntOrNull(maxRaw);
        return (min != null || max != null) ? { min, max } : null;
      })
      .filter(Boolean);
    const subscriberRanges = [
      ...subscriberTiersList.map((t) => getSubscriberTierRange(t)).filter(Boolean),
      ...subscriberRangePairs,
    ];
    const requestedCategory = cleanStr(q.category || q.niche);
    ...
    const mongoLimit = frontendPagination
      ? Math.min(limitCap, Math.max(limit, 50))
      : limit;
    ...
    // Exact-count target (e.g. "give me 30 dashcam creators"). Bounded to limitCap
    // (300) to protect YouTube API quota on rare niches; no artificial floor when
    // the user explicitly asks for fewer than 25.
    const minimumResults = Math.min(limitCap, Math.max(1, toIntOrNull(q.minimumResults) || 25));
    ...
    if (directChannelSearch) {
      const directResult = await runDirectChannelSearch({
        ...
        minAvgViews,
        strictFilters,
        // Respect the user's exact-count target (minimumResults) directly, rather
        // than the Mongo-cache-lookup buffer size (mongoLimit), which always floors at 50.
        limit: Math.min(limitCap, Math.max(limit, minimumResults)),
        sort,
        context,
      });
...
const STATIC_TIER_ID_MAP = {
  // Fidem default seed ids. Keep this as a safe fallback when campaign details are not populated.
...
const STATIC_COUNTRY_ID_MAP = {
  // Known Fidem seed ids seen in production examples.
...
/* -------------------------------------------------------------------------- */
/*         Real audience demographics (Modash) for the YouTube media kit      */
/* -------------------------------------------------------------------------- */
...
function getYouTubeHandleFromCreator(creator = {}) {
  const fromUrl = String(creator.channelUrl || '').match(/youtube\.com\/@([^/?#]+)/i)?.[1];
  return cleanStr(fromUrl || '').replace(/^@+/, '');
}

async function findCachedModashYouTubeProfile(creator = {}) {
  if (!ModashProfile) return null;

  const channelId = cleanStr(creator.channelId);
  const handle = getYouTubeHandleFromCreator(creator);
  if (!channelId && !handle) return null;

  const or = [];
  if (channelId) or.push({ userId: channelId });
  if (handle) {
    const rx = exactCaseInsensitive(handle);
    const handleRx = exactCaseInsensitive(`@${handle}`);
    or.push({ username: rx }, { handle: rx }, { handle: handleRx });
  }
  if (!or.length) return null;

  return ModashProfile.findOne({ provider: 'youtube', $or: or }).lean();
}

// Merge real Modash audience data (if cached) into the media kit's audienceInsights.
// Falls back to the existing YouTube-inferred estimate when nothing is cached yet —
// this only reads from the DB, never calls the Modash API, so it's free.
async function applyModashAudienceDemographics(mediaKit, creator) {
  const profile = await findCachedModashYouTubeProfile(creator).catch(() => null);
  const audience = profile?.audience;

  if (!audience) {
    mediaKit.audienceInsights = {
      ...mediaKit.audienceInsights,
      demographicsSource: 'estimated',
    };
    return mediaKit;
  }
  ...
  mediaKit.audienceInsights = {
    ...mediaKit.audienceInsights,
    demographicsSource: 'modash',
    genderSplit: (audience.genders || []).map((g) => ({ label: g.code || g.name, percentage: Math.round((g.weight || 0) * 100) })),
    ageSplit: (audience.ages || []).map((a) => ({ label: a.code || a.name, percentage: Math.round((a.weight || 0) * 100) })),
    topCountries: (audience.geoCountries || []).slice(0, 5).map((c) => ({ country: c.name || c.code, percentage: Math.round((c.weight || 0) * 100) })),
    interestCategories: (audience.interests || []).length
      ? audience.interests.slice(0, 10).map((i) => i.name).filter(Boolean)
      : mediaKit.audienceInsights?.interestCategories,
    credibilityScore: typeof audience.credibility === 'number' ? Math.round(audience.credibility * 100) : null,
  };

  return mediaKit;
}

async function fetchRealAudienceDemographics(req, res) {
  try {
    ...
    if (!modashHelpers.searchForUsername || !modashHelpers.modashGET || !modashHelpers.normalizeReportData || !modashHelpers.upsertModashProfileFromReport) {
      return res.status(500).json({ success: false, error: 'Modash integration is not available' });
    }
    ...
    const handle = getYouTubeHandleFromCreator(creator);
    if (!handle) {
      return res.status(400).json({ success: false, error: 'This creator has no resolvable YouTube handle for Modash lookup' });
    }

    const hit = await modashHelpers.searchForUsername('youtube', handle);
    if (!hit || !hit.userId) {
      return res.status(404).json({ success: false, error: 'This creator was not found on Modash' });
    }

    const reportJSON = await modashHelpers.modashGET(
      `/youtube/profile/${encodeURIComponent(hit.userId)}/report`,
      { calculationMethod: 'median' }
    );
    const normalized = modashHelpers.normalizeReportData(reportJSON);
    await modashHelpers.upsertModashProfileFromReport(normalized, 'youtube', { userIdFromRequest: hit.userId });

    const cachedMediaKit = InfoMediaKit ? await InfoMediaKit.findOne({ channelId }).lean() : null;
    const baseMediaKit = cachedMediaKit?.mediaKitData || buildBrandMediaKitData(creator, {});
    const mediaKit = await applyModashAudienceDemographics({ ...baseMediaKit }, creator);

    if (InfoMediaKit) {
      InfoMediaKit.updateOne(
        { channelId },
        { $set: { mediaKitData: mediaKit, lastOpenedAt: new Date() } }
      ).catch(() => {});
    }

    return res.status(200).json({ success: true, data: mediaKit });
  } catch (err) {
    await saveErrorLog(req, err, err?.status || 500, 'YOUTUBE_MEDIA_KIT_REAL_AUDIENCE');
    return res.status(err?.status || 500).json({
      success: false,
      error: err?.message || 'Failed to fetch real audience demographics',
    });
  }
}
  ...
  try {
    ...
    if (InfoMediaKit) {
      ...
      if (cachedMediaKit?.mediaKitData && hasUsableMediaKitVideoData(cachedMediaKit.mediaKitData)) {
        ...
        let cachedData = attachTopLevelEmailToMediaKit(
          cachedMediaKit.mediaKitData,
          cachedMediaKit
        );

        // Older cached media kits (from before real-demographics support) won't have
        // demographicsSource set yet — backfill it with the same free cache-only check.
        if (!cachedData.audienceInsights?.demographicsSource) {
          cachedData = await applyModashAudienceDemographics(cachedData, cachedMediaKit);
          InfoMediaKit.updateOne({ channelId }, { $set: { mediaKitData: cachedData } }).catch(() => {});
        }
    ...
    // Free, cache-only check: merge real Modash audience demographics if this
    // creator was already resolved on Modash before. Never calls the live
    // Modash API here (that only happens via the explicit "Get Real Audience
    // Data" action), so this costs nothing beyond one extra Mongo lookup.
    await applyModashAudienceDemographics(mediaKit, fastCreator);
  ...
  try {
    ...
    try {
      const imageRes = await fetch(rawUrl, {
        ...
        signal: ac.signal,
        headers: {
          'user-agent': 'Mozilla/5.0 Fidem Image Proxy',
