...
const EMAIL_ATTACHMENT_BUCKET =
  process.env.EMAIL_ATTACHMENT_BUCKET || "fidem";
...
async function sendEmail({
  ...
  const fixedFrom = cleanEmail(
    from || process.env.SES_FROM_EMAIL || "confirm@fidem.com"
  );
...
async function uploadEmailRecordToS3(record) {
  ...
  const Key = `fidem-email-archive/outbound/marketing/${date}/${id}.json`;
