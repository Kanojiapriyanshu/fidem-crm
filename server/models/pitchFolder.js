...
// One entry per brand a folder is shared with, so the admin can tell which
// brand received which link (the legacy singular `share` field below only
// ever held one reused link for everyone).
const FolderBrandShareSchema = new mongoose.Schema(
  {
...
const PitchFolderSchema = new mongoose.Schema(
  {
    ...
    // Per-brand share links (new). `share` above is kept untouched for
    // backward compatibility with links already handed out before this
    // existed — new shares always go here instead.
    shares: {
      type: [FolderBrandShareSchema],
      default: () => [],
    },
